...
public class GameStartUp {
    ...
    private static List<GameThread> createGameThreads() {
        ...
        for (int i = 0; i < threadCount; i++) {
            ...
            GameProcess.addGameThread(gameThread);
            gameThread.start();
    ...
    private static void assignServicesToThreads(List<GameServiceBase> services) {
        int progressThreadCount = GameProcess.getThreadCount();
        for (GameServiceBase service : services) {
            ...
            if (targetThread == null) {
                LogCore.logger.error("无法分配GameService到GameThread: {}", service.getName());
                throw new RuntimeException("无法分配GameService到GameThread: " + service.getName());
            }
