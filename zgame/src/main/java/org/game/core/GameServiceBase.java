...
public abstract class GameServiceBase {
    ...
    public void registerService() {
        ...
        // 获取当前线程名称
        String gameThreadName = GameThread.getCurrentThreadName();
        // 使用 ServiceRegistryManager 注册服务
        if (!ServiceRegistryManager.registerService(serviceName, gameThreadName)) {
            throw new RuntimeException("Service already registered: " + serviceName);
        }
        logger.info("Service registered: {} on thread: {}", serviceName, gameThreadName);
    }
